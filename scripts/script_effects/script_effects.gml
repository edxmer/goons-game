function effect_update(goon_id)
{
	...
	goon_effect_ids_other_add(goon_id)
}
...
function goon_effect_ids_other_add(goon_id)
{
	if goon_id.verboseness<=0
	{
		array_push(goon_id.active_effect_list,"muted")
	}
	if goon_id.blue 
	{
		array_push(goon_id.active_effect_list,"blue")
	}
	if goon_id.dumb 
	{
		array_push(goon_id.active_effect_list,"dumb")
	}
...
function effect_tick(goon_id)
{
	...
	if goon_id.has_effects
	{
		...
		if goon_id.effects.grid_mode.is
		{
			...
			{
				global.grid_mode_item=goon_id.inventory
		...
		if goon_id.effects.pick_up_building.is{
			if pickup_item_penalty==0 && !item_tags_contains(goon_id.inventory,"station")
			{
